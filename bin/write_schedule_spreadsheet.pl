...
my $excel_file = "/home/keith/Reporting/public/reports/recharge/schedule_report.xls";
...
my $query = qq{SELECT * FROM v_weekly_schedule_report};
#print "$query\n";

# Build a query...
my $sth = $dbh->prepare($query);
$sth->execute();
my $worksheet = $workbook->addworksheet("Schedule Report");
my $row = 3;
$worksheet->write(0,0, "Date Created", $format);
$worksheet->write(1,0, DateTime->now( time_zone => 'Australia/Sydney' ), $format);
$worksheet->write(2,0, "Sites including Spinoza and Sole Risk", $format);
